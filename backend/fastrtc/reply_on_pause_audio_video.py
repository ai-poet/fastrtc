...
from typing import Any, AsyncGenerator, Callable, Generator, Literal, cast, List
...
from .tracks import EmitType, AudioVideoStreamHandler, VideoEmitType
...
class AppState:
    ...
    last_video_frame_time: float = 0.0

    def new(self):
        new_state = AppState()
        new_state.last_video_frame_time = self.last_video_frame_time
        return new_state
...
class ReplyOnPauseAudioVideo(AudioVideoStreamHandler):
    """
    ...
    def __init__(
        ...
        self.video_frames_base64: List[str] = []
    ...
    def encode_image(self, data: np.ndarray) -> str:
        """将图像数据编码为base64字符串。"""
    ...
    def copy(self):
        """Creates a new instance of ReplyOnPauseAudioVideo with the same configuration."""
        instance = ReplyOnPauseAudioVideo(
            self.fn,
            self.startup_fn,
            self.algo_options,
            self.model_options,
        ...
        # 复制视频帧数据
        instance.video_frames_base64 = self.video_frames_base64.copy()
        return instance
    ...
    def emit(self):
        ...
        if not self.event.is_set():
            ...
            if not self.generator:
                ...
                if self._needs_additional_inputs:
                    self.latest_args[0] = (self.state.sampling_rate, audio, self.video_frames_base64)
                    self.generator = self.fn(*self.latest_args)  # type: ignore
                else:
                    self.generator = self.fn((self.state.sampling_rate, audio, self.video_frames_base64))  # type: ignore
            ...
            try:
                ...
                traceback.print_exc()
                logger.debug("Error in ReplyOnPauseAudioVideo: %s", e)
                self.reset()
                raise e

    def video_receive(self, frame: np.ndarray) -> None:
        """
        Process an incoming video frame synchronously.
        每1秒采样一帧，并保存为base64编码格式，保持最新的2帧。

        Args:
            frame: The incoming video frame as numpy array.
        """
        current_time = time.time()
        if current_time - self.state.last_video_frame_time >= 1.0:
            frame_base64 = self.encode_image(frame)
            
            self.video_frames_base64.append(frame_base64)
            if len(self.video_frames_base64) > 2:
                self.video_frames_base64.pop(0)
                
            self.state.last_video_frame_time = current_time
            logger.debug("Captured video frame at %s", current_time)
